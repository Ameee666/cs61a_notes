...
def about(subject):
    ...
    "*** YOUR CODE HERE ***"
    def f(paragraph):
        m=split(remove_punctuation(lower(paragraph)))
        return any(word in m for word in subject )
...
def accuracy(typed, source):
    ...
    if typed == '' or source=='':
        ...
        m=list(zip([p for p in typed_words],[y for y in source_words]))
        return len([p for p in m if p[0] == p[1] ])/len(typed_words)*100.0
...
def furry_fixes(typed, source, limit):
    ...
    if typed == "" or source == "":
        ...
        return float('inf') if diff > limit else diff
...
def report_progress(typed, source, user_id, upload):
    ...
    "*** YOUR CODE HERE ***"
    ans=0
    for m in zip(typed,source):
        if m[0]==m[1]:
            ans+=1
        else:
            break
    progress0=ans/len(source)
    dict0={}
    dict0['id']=user_id
    dict0['progress']=progress0
    upload(dict0)
    return progress0
...
def time_per_word(words, timestamps_per_player):
    ...
    def time_maker(tpp):
        def munus(list0):
            return [list0[k+1]-list0[k] for k in range(len(list0)-1)]
        return [munus(tpp[k]) for k in range(len(tpp))]
    times=time_maker(timestamps_per_player)
    # END PROBLEM 9
    ...
    def find_small(s):
        if len(s)==1:
            ...
            for p in range(len(s)-1):
                if s[p] <= s[p+1:][find_small(s[p+1:])]:
                    ans=p
                    break
            return min(len(s)-1,ans)
    ans=[[]for k in range(len(words_and_times['times']))]
    colomns=[[words_and_times['times'][i][k] for i in range(len(words_and_times['times']))] for k in range(len(words_and_times['words']))]
    for number in range(len(colomns)):
        m= find_small(colomns[number])
        ans[m].append(words_and_times['words'][number])
    return ans
