...
def balanced(m):
    ...
    def mass_get(m,n):
        if is_mobile(m):
            ...
            n=total_mass(m)
            return n
    leftmass,rightmass=0,0
    if mass_get(end(left(m)),leftmass)*length(left(m))==mass_get(end(right(m)),rightmass)*length(right(m)):
        return True
    else:
        return False
"""注意括号开闭，一定保证括号的正确性"""
...
def max_path_sum(t):
    ...
    if branches(t):
        for branch in branches(t):
            sublength.append(max_path_sum(branch))
        return pril+max(sublength)
    else:
        return pril
